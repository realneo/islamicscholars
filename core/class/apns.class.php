	...
	*/
	...
	function __construct($args=NULL, $certificate=NULL, $sandboxCertificate=NULL, $logPath=NULL)
	{
		if(!empty($certificate) && file_exists($certificate))
			...
			$this->certificate = FANWE_ROOT.'./public/pem/apns.pem';

		if(!empty($sandboxCertificate) && file_exists($sandboxCertificate))
			$this->sandboxCertificate = $sandboxCertificate;
		else
			$this->sandboxCertificate = FANWE_ROOT.'./public/pem/apns-dev.pem';
		...
		if ($logPath !== null)
			...
			$this->logPath = FANWE_ROOT.'./public/logs/apns.log';
	...
	private function _unregisterDevice($token){
		$sql = "UPDATE ".FDB::table('apns_devices')." 
	...
	private function _fetchMessages()
	{
		...
		$sql = "SELECT
			...
			FROM ".FDB::table('apns_messages')." as apns_messages
			LEFT JOIN ".FDB::table('apns_devices')." as apns_devices ON (`apns_devices`.`pid` = `apns_messages`.`fk_device` AND `apns_devices`.`clientid` = `apns_messages`.`clientid`)
	...
	private function _pushSuccess($pid){
		$sql = "UPDATE ".FDB::table('apns_messages')." 
	...
	public function newMessage($fk_device=NULL, $delivery=NULL, $clientId=NULL){
		...
		{
			$sql = "SELECT `pid` FROM ".FDB::table('apns_devices')." WHERE `status`='active'";

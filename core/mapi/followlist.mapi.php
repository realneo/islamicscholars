...
class followlistMapi
{
	public function run()
	{
		...
		$total = FDB::resultFirst('SELECT COUNT(uid) FROM '.FDB::table('user_follow').' WHERE f_uid = '.$uid);
		$page_size = PAGE_SIZE;
		...
		$res = FDB::query('SELECT u.uid,u.user_name,u.avatar,uc.fans,up.reside_province,up.reside_city,up.introduce 
				...
				WHERE uf.f_uid = '.$uid.' ORDER BY uf.create_time DESC LIMIT '.$limit);

...
class NoticeService
{
	...
	public function send($data)
	{
		...
		FS('User')->updateNotice($data['uid'],6);
	}
	...
	{
		return FDB::resultFirst('SELECT COUNT(uid) FROM '.FDB::table('sys_notice').' WHERE uid = '.(int)$uid);
	}
	
	public function getList($uid,$limit = '0,20')
	{
		$uid = (int)$uid;
		$list = FDB::fetchAll('SELECT * FROM '.FDB::table('sys_notice').' WHERE uid = '.$uid.' ORDER BY id DESC LIMIT '.$limit);

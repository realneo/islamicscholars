...
class SharegoodsService{
	...
	public function __construct($url)
	{
        ...
        $domain = implode('.',$host);
		$class = FDB::resultFirst("select `class` from ".FDB::table('sharegoods_module')." where domain like '%".$domain."%' and status = 1  limit 1");

		if(empty($class))
		{
			$shop_id = (int)FDB::resultFirst("select id from ".FDB::table('yiqifa_shop')." where domain like '%".$domain."%' and status = 1  limit 1");
			if($shop_id > 0)
			{
				$class = 'yiqifa';

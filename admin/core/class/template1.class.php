...
class Template {
	...
	function parseTemplate($tpl_file, $tpl_dir, $file, $cache_file)
	{
		...
		$template = preg_replace("/\{lang\s+(\S+?)\}/ies", "\$this->languageVar('\\1')", $template);
		$template = preg_replace("/\{sprintf\s+(\S+?)(?:\s+(\S+?))?\}/ies", "\$this->sprintfTags('\\1','\\2')", $template);
		$template = preg_replace("/\{nl2br\s+(\S+?)\}/ies", "\$this->nl2brTags('\\1')", $template);
		$template = preg_replace("/[\n\r\t]*\{advlayout\s+id=(\S+)(?:\scount=(\S+))*(?:\starget=(\S+?))?\}[\n\r\t]*/is", "<!--dynamic advLayout args=\\1,\\2,\\3-->", $template);
		$template = preg_replace("/[\n\r\t]*\{advlayout\s+name='(\S+?)'(?:\stmpl='(\S+?)')*(?:\starget='(\S+?)')?\}[\n\r\t]*/is", "<!--dynamic advLayoutName args=\\1,\\2,\\3-->", $template);
		$template = preg_replace("/[\n\r\t]*\{getimg\s+(\S+?)(?:\s+(\S+?)\s+(\S+?)(?:\s+(\S+?)(?:\s+(\S+?))?)?)?\}[\n\r\t]*/ies", "\$this->getImgTags('\\1','\\2','\\3','\\4','\\5')", $template);
		$template = preg_replace("/[\n\r\t]*\{getuser\s+(\S+?)\s+(\d+?)(?:\s+'(.*?)')?(?:\s+'(.*?)')?(?:\s+'(.*?)')?\}[\n\r\t]*/ies", "\$this->getUserTags('\\1','\\2','\\3','\\4','\\5')", $template);
		$template = preg_replace("/[\n\r\t]*\{getfollow\s+(\S+?)\s+(\S+?)\}[\n\r\t]*/is", "<!--getfollow \\1 \\2-->", $template);
		$template = preg_replace("/[\n\r\t]*\{date\s+(\S+?)(?:\s+(\S+?))?\}[\n\r\t]*/ie", "\$this->dateTags('\\1','\\2')", $template);
		$template = preg_replace("/[\n\r\t]*\{timelag\s+(\S+?)\}[\n\r\t]*/ie", "\$this->timeLagTags('\\1')", $template);
		$template = preg_replace("/[\n\r\t]*\{avatar\s+(\S+?)(?:\s+(\S+?))?(?:\s+(\S+?))?\}[\n\r\t]*/ies", "\$this->avatarTags('\\1','\\2','\\3')", $template);
		$template = preg_replace("/[\n\r\t]*\{cutstr\s+(\S+?)\s+(\S+?)(?:\s+(\S+?))?\}[\n\r\t]*/ies", "\$this->cutstrTags('\\1','\\2','\\3')", $template);
	...
	function avatarTags($id,$type = 's',$is_src = 0)
	{
		if(empty($code))
			$code = '"'.$code.'"';
		...
		$this->replace_code['replace'][$i] = "<?php echo avatar($id,'$type',$is_src);?>";
		return $search;
	}
	 ...
	 * @return string
	 */ 
	function getImgTags($id,$width=0,$height=0,$gen=0,$is_full=0)
	{
		if(preg_match('/\$.+/i',$id))
			$id = $id;
		else

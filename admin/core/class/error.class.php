...
class FanweError
{
	...
	function debugBacktrace()
	{
		...
		{
			$fanwe_root = str_replace('/','\\',FANWE_ROOT);
			$file = str_replace($fanwe_root, '', $error['file']);
			if(strpos($file,'fanwe.service.php') === FALSE)
			{
	...
	function dbError($message, $sql)
	{
		...
		$showerrs = array('not_connect','config_not_found');
		
		if(SYS_DEBUG || in_array($message,$showerrs))
		{

...
class LookModule
{
	public function index()
	{
		...
		if(getCacheIsUpdate($cache_file,SHARE_CACHE_TIME,1))
		{
			...
			$_FANWE['nav_title'] = lang('common','look');
			...
			$hot1_url['url'] = FU('look/'.ACTION_NAME,$sort_page_args);
			if($sort=='hot1')
				$hot1_url['act'] = 1;
	
			$sort_page_args['sort'] = 'hot7';
			$hot7_url['url'] = FU('look/'.ACTION_NAME,$sort_page_args);
			if($sort=='hot7')
				$hot7_url['act'] = 1;
	
			$sort_page_args['sort'] = 'new';
			$new_url['url'] = FU('look/'.ACTION_NAME,$sort_page_args);
			if($sort=='new')
				$new_url['act'] = 1;
			...
			if($type == 'goods')
			{
				...
				$sql_count = 'SELECT COUNT(DISTINCT share_id) FROM '.FDB::table('share_look_goods');
			}
			else
			{
				$sql = 'SELECT DISTINCT(share_id) FROM '.FDB::table('share_look_index').$sort;
				$sql_count = 'SELECT COUNT(DISTINCT share_id) FROM '.FDB::table('share_look_index');
			}
	
			$page_size = (int)$_FANWE['setting']['share_pb_item_count'] * (int)$_FANWE['setting']['share_pb_load_count'];
			$count = FDB::resultFirst($sql_count);
	
			$pager = buildPage('look/'.ACTION_NAME,$page_args,$count,$_FANWE['page'],$page_size,'',3);
			$page_args['page'] = $_FANWE['page'];
			$page_args['pindex'] = '_pindex_';
			$pb_url = $_FANWE['site_root'].'services/service.php?m=share&a=look&'.http_build_query($page_args);
			$pb_list = array();
			...
			$look_darens = FS('Daren')->getDarensByType(1);

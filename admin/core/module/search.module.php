...
class SearchModule
{
	...
	public function mall()
	{
		...
		$_FANWE['nav_title'] = lang('common','search_mall') . $_FANWE['nav_title'];
		$keyword = trim(urldecode($_FANWE['request']['keyword']));
		
		$mall_list = array();
		$is_empty = false;
		if(empty($keyword) || $keyword == lang('template','search_mall'))
			$is_empty = true;
		else
		{
			$match_key = FS('Words')->segmentToUnicode($keyword,'+');
		...
		if(!$is_empty)
		{
			...
			switch($sort)
			{
				//最热
				case 'hot':
					$sort = " ORDER BY m.fensi DESC ";
				break;
				//最新
				case 'new':
					$sort = " ORDER BY m.mall_id DESC";
				break;
			...
			//$where = " WHERE match(m.name) against('".$match_key."' IN BOOLEAN MODE) or match(m.des) against('".$match_key."' IN BOOLEAN MODE) ";
			$where = " WHERE m.name like '%".$keyword."%' or m.des like '%".$keyword."%' ";
			$sql_count = 'SELECT COUNT(m.mall_id) FROM '.FDB::table('mall').' AS m ';
			$append_sql = '';
			
			error_log('[search.module.php]405----------------sql is: '.$sql_count.$where,0);
			$count = (int)FDB::resultFirst($sql_count.$where);
			error_log('[search.module.php]405----------------count is: '.$count,0);
			...
			if($count > 0)
			{
				...
				$sql = 'SELECT m.* FROM '.FDB::table('mall').' AS m '.$append_sql.$where.$sort.' LIMIT '.$pager['limit'];
			error_log('[search.module.php]413----------------sql is :'.$sql,0);
				
				$res = FDB::query($sql);
				while($data = FDB::fetch($res))
				{
					$mall_list[$data['mall_id']] = $data;
				}
			}
		}
		...
		$keyword = htmlspecialchars($keyword);
		include template('page/search/search_mall');
		display();
	}
	...
	public function brand()
	{
		...
		$_FANWE['nav_title'] = lang('common','search_brand') . $_FANWE['nav_title'];
		$keyword = trim(urldecode($_FANWE['request']['keyword']));
		
		
		$mall_list = array();
		$is_empty = false;
		if(empty($keyword) || $keyword == lang('template','search_brand'))
			$is_empty = true;
		else
		{
			$match_key = FS('Words')->segmentToUnicode($keyword,'+');
		...
		if(!$is_empty)
		{
			...
			//$where = " WHERE match(m.name) against('".$match_key."' IN BOOLEAN MODE) or match(m.des) against('".$match_key."' IN BOOLEAN MODE) ";
			$where = " WHERE m.bname like '%".$keyword."%' or m.des like '%".$keyword."%' ";
			$sql_count = 'SELECT COUNT(m.bid) FROM '.FDB::table('brand').' AS m ';
			$append_sql = '';
			...
			if($count > 0)
			{
				...
				$sql = 'SELECT m.* FROM '.FDB::table('brand').' AS m '.$append_sql.$where.$sort.' LIMIT '.$pager['limit'];
				
			error_log('[search.module.php]511----------------sql is :'.$sql,0);
				$res = FDB::query($sql);
				while($data = FDB::fetch($res))
				{
		...
		$keyword = htmlspecialchars($keyword);
		include template('page/search/search_brand');
		display();
	}
	...
	public function discount()
	{
		...
		$_FANWE['nav_title'] = lang('common','search_discount') . $_FANWE['nav_title'];
		$keyword = trim(urldecode($_FANWE['request']['keyword']));
		
		
		$mall_list = array();
		$is_empty = false;
		if(empty($keyword) || $keyword == lang('template','search_discount'))
			$is_empty = true;
		else
		{
			$match_key = FS('Words')->segmentToUnicode($keyword,'+');
		...
		if(!$is_empty)
		{
			...
			//$where = " WHERE match(m.name) against('".$match_key."' IN BOOLEAN MODE) or match(m.des) against('".$match_key."' IN BOOLEAN MODE) ";
			$where = " WHERE m.dname like '%".$keyword."%' or m.ddes like '%".$keyword."%'";
			$where2 = " and m.d_brandid=b.bid ";
			$sql_count = 'SELECT COUNT(m.did) FROM '.FDB::table('discount').' AS m ';
			$append_sql = '';
			...
			if($count > 0)
			{
				...
				$sql = 'SELECT m.*,b.blogo FROM '.FDB::table('discount').' AS m,meixun_brand as b '.$append_sql.$where.$where2.$sort.' LIMIT '.$pager['limit'];
				
			error_log('[search.module.php]511----------------sql is :'.$sql,0);
				$res = FDB::query($sql);
				while($data = FDB::fetch($res))
				{
					$mall_list[$data['did']] = $data;
				}
			}
		}
		
		$keyword = htmlspecialchars($keyword);
		include template('page/search/search_discount');
		display();
	}
	...
	public function res()
	{
		...
		$_FANWE['nav_title'] = lang('common','search_res') . $_FANWE['nav_title'];
		$keyword = trim(urldecode($_FANWE['request']['keyword']));
		
		
		$mall_list = array();
		$is_empty = false;
		if(empty($keyword) || $keyword == lang('template','search_res'))
			$is_empty = true;
		else
		{
			$match_key = FS('Words')->segmentToUnicode($keyword,'+');
		...
		if(!$is_empty)
		{
			...
			//$where = " WHERE match(m.name) against('".$match_key."' IN BOOLEAN MODE) or match(m.des) against('".$match_key."' IN BOOLEAN MODE) ";
			$where = " WHERE m.name like '%".$keyword."%' or m.intro like '%".$keyword."%' or ri.name like '%".$keyword."%' or ri.intro like '%".$keyword."%'";
			$where2 = " and m.id=ri.parent_id ";
			$sql_count = 'SELECT COUNT(distinct m.id) FROM '.FDB::table('restaurant').' AS m ,meixun_resitem as ri';
			$append_sql = '';
			...
			if($count > 0)
			{
				...
				$sql = 'SELECT m.* FROM '.FDB::table('restaurant').' AS m,meixun_resitem as ri '.$append_sql.$where.$where2.$sort.' LIMIT '.$pager['limit'];
				
			error_log('[search.module.php]511----------------sql is :'.$sql,0);
				$res = FDB::query($sql);
				while($data = FDB::fetch($res))
				{
					$mall_list[$data['id']] = $data;
				}
			}
		}
		
		$keyword = htmlspecialchars($keyword);
		include template('page/search/search_res');
		display();
	}
	public function tuan()
	{
		...
		$_FANWE['nav_title'] = lang('common','search_tuan') . $_FANWE['nav_title'];
		$keyword = trim(urldecode($_FANWE['request']['keyword']));
		
		
		$mall_list = array();
		$is_empty = false;
		if(empty($keyword) || $keyword == lang('template','search_tuan'))
			$is_empty = true;
		else
		{
			$match_key = FS('Words')->segmentToUnicode($keyword,'+');
		...
		if(!$is_empty)
		{
			...
			//$where = " WHERE match(m.name) against('".$match_key."' IN BOOLEAN MODE) or match(m.des) against('".$match_key."' IN BOOLEAN MODE) ";
			$where = " WHERE m.name like '%".$keyword."%' or m.des like '%".$keyword."%' or m.pname like '%".$keyword."%' ";
			$sql_count = 'SELECT COUNT(distinct m.id) FROM '.FDB::table('tuan').' AS m ';
			$append_sql = '';
			...
			if($count > 0)
			{
				...
				$sql = 'SELECT m.* FROM '.FDB::table('tuan').' AS m '.$append_sql.$where.$where2.$sort.' LIMIT '.$pager['limit'];
		...
		$keyword = htmlspecialchars($keyword);
		include template('page/search/search_tuan');
		display();
	}
	...
	private function formatSearchNavs(&$navs,$keyword)
	{
		...
		$navs['mall'] = FU('search/mall',$args);
		$navs['brand'] = FU('search/brand',$args);
		$navs['discount'] = FU('search/discount',$args);
		$navs['res'] = FU('search/res',$args);
		$navs['tuan'] = FU('search/tuan',$args);
	}
}
?>
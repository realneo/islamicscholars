...
class AlbumModule
{
	public function index()
	{
		...
		if(getCacheIsUpdate($cache_file,SHARE_CACHE_TIME,1))
		{
			...
			$sql = 'SELECT COUNT(DISTINCT share_id) FROM '.FDB::table('album_share_index');
			...
			if($count > 0)
			{
				...
				$pager = buildPage('album/'.ACTION_NAME,$page_args,$count,$_FANWE['page'],$page_size,'',3);
				$page_args['page'] = $_FANWE['page'];
				$page_args['pindex'] = '_pindex_';
				$pb_url = $_FANWE['site_root'].'services/service.php?m=album&a=index&'.http_build_query($page_args);
				$pb_list = array();
				...
				$limit = ' LIMIT '.($_FANWE['page'] - 1) * $pager['page_size'] . "," . $_FANWE['setting']['share_pb_item_count'];
	...
	public function category()
	{
		...
		if(getCacheIsUpdate($cache_file,SHARE_CACHE_TIME,1))
		{
			...
			$sql = 'SELECT COUNT(DISTINCT share_id) FROM '.FDB::table('album_share_index').$where;
			$count = FDB::resultFirst($sql);
			$share_list = array();
			$share_display = array();
			...
			if($count > 0)
			{
				...
				$page_args['page'] = $_FANWE['page'];
				$page_args['pindex'] = '_pindex_';
				$pb_url = $_FANWE['site_root'].'services/service.php?m=album&a=category&'.http_build_query($page_args);
				$pb_list = array();
				if($count > $_FANWE['setting']['share_pb_item_count'])
				{
					for($i = 2;$i <= $_FANWE['setting']['share_pb_load_count'];$i++)
				...
				$res = FDB::query('SELECT DISTINCT share_id FROM '.FDB::table('album_share_index').$where.$order.$limit);
				while($data = FDB::fetch($res))
				{
					$share_list[$data['share_id']] = false;
				}

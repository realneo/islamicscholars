...
class GroupService
{
	...
	public function createGroup($data)
	{
		...
		if($group['cid'] > 0)
		{
			FDB::query('UPDATE '.FDB::table('forum_category').' SET forum_count = forum_count + 1 WHERE id = '.$group['cid']);
	...
	public function removeGroup($fid)
	{
		...
		if($group['cid'] > 0)
		{
			FDB::query('UPDATE '.FDB::table('forum_category').' SET forum_count = forum_count - 1 WHERE id = '.(int)$group['cid']);
			GroupService::updateCateCacheGroups($group['cid']);
		}
	...
	public function updateCateCacheGroups($cid)
	{
		...
		FDB::update('forum_category',array('cache_data'=>$cache_data),'id = '.$cid);
	}
	...
	public function getNewGroupUsers($fid,$num = 6)
	{
		...
		$list = array();
		$res = FDB::query('SELECT uid FROM '.FDB::table('forum_users').' WHERE fid = '.$fid.' AND type = 0 ORDER BY create_time ASC');
		while($data = FDB::fetch($res))
		{
			$list[] = $data['uid'];
		}

...
class ShareService
{
	...
	public function submit($_POST,$is_check = true,$is_score = true)
	{
		...
		/*adding the brand info for the sharing goods Guang 0630 */
		$share_data['brand'] = isset($_POST['brand']) ? htmlspecialchars(trim($_POST['brand'])) : '';
		$share_data['local'] = isset($_POST['local']) ? htmlspecialchars(trim($_POST['local'])) : '';
		$share_data['goodnumber'] = isset($_POST['goodnumber']) ? htmlspecialchars(trim($_POST['goodnumber'])) : '';
		
		/* add the following attributes for the fb fucntion. */
		//$share_data['dpcat'] = isset($_POST['dpcat']) ? htmlspecialchars(trim($_POST['dpcat'])) : '';
		//$share_data['dpprice'] = isset($_POST['dpprice']) ? htmlspecialchars(trim($_POST['dpprice'])) : '';
		$share_data['dptype'] = isset($_POST['dptype']) ? htmlspecialchars(trim($_POST['dptype'])) : '';
		$share_data['dptitile'] = isset($_POST['dptitile']) ? htmlspecialchars(trim($_POST['dptitile'])) : '';
		$share_data['dpcontent'] = isset($_POST['content']) ? htmlspecialchars(trim($_POST['content'])) : '';
		...
		for($i=1;$i<=$goodsnum;$i++) {
			$dp_data= array();
			$dp_data['shareid']= $result ['share_id'];
		
			$varname1 = "dpcat".$i;
			$varname2 = "brand".$i;
			$varname3 = "local".$i;
			$varname4 = "dpprice".$i;
			$varname5 = "goodnumber".$i;
		/**/
			$dp_data['dpcat'] = isset($_POST[$varname1]) ? htmlspecialchars(trim($_POST[$varname1])) : '';
			$dp_data['brand'] = isset($_POST[$varname2]) ? htmlspecialchars(trim($_POST[$varname2])) : '';
			$dp_data['local'] = isset($_POST[$varname3]) ? htmlspecialchars(trim($_POST[$varname3])) : '';
			$dp_data['dpprice'] = isset($_POST[$varname4]) ? htmlspecialchars(trim($_POST[$varname4])) : '';
			$dp_data['goodnumber'] = isset($_POST[$varname5]) ? htmlspecialchars(trim($_POST[$varname5])) : '';
			
			//$sqldp = "insert into meixun_share_dp(shareid,brand,local,goodnumber,dpcat,dpprice)   values(".$shareID.",".$brand.",".$local.",".$goodnumber.",".$dpcat.",".$dpprice.")";
			
			error_log("[core-service-share.service.php]submit--->the sql".$i." is ok.",0);
			FDB::insert("share_dp",$dp_data);
		}
		return $result;
	}
				...
				foreach($share_goods as $goods)
				{
					...
					if($goods['goods_id'] == 0)
					{
						...
						if(!empty($o_img))
						{
							...
							/*Insert into the goods table.Guang 0630  */
	...
	public function deleteShare($share_id,$is_score = true)
	{
		...
		if(!empty($share))
		{
			...
			else
			{
				...
				if(in_array($share['type'],array('default','ask','comments','ask_post','bar','bar_post')))
				{
	...
	{
	//error_log("[share.service.php]000",0);
	...
	//error_log("[share.service.php]111",0);
	...
	//error_log("[share.service.php]222",0);
	...
	/* 增加评论。*/
	public function saveComment2($_POST)
	{
		global $_FANWE;
		$share_id = intval($_POST['share_id']);
		//$share = ShareService::getShareById($share_id);
		...
		$data['userid'] = $_FANWE['uid'];
		$data['parentid'] = $share_id;
		//$data['share_id'] = $share_id;
		$data['ctime'] = TIME_UTC;
		$comment_id = FDB::insert('comments',$data,true);
	...
	public function getShareComment2($comment_id)
	{
		return FDB::fetchFirst('SELECT *
			FROM '.FDB::table("comments").'
			WHERE id = '.$comment_id);
	}

...
function fimport($file_name, $folder = '')
{
	...
	//error_log("[global.func.php]55",0);
...
function FS($service)
{
	//error_log("[global.func.php]fs:102",0);
	
	static $services = array();
	if($services[$service] === NULL)
	{
	//error_log("[global.func.php]fs:107 service is:".$service,0);
	$guang = fimport("service/".strtolower($service));
	
	//error_log("[global.func.php]fs:110 fimport is:".$guang,0);
		require_once $guang;
		
	//error_log("[global.func.php]fs:113",0);
	
		$s = ucfirst($service)."Service";
	//error_log("[global.func.php]fs:114",0);
	
		$services[$service] = new $s();
		
	//error_log("[global.func.php]fs:118",0);
		unset($s);
	}

	//error_log("[global.func.php]fs:222",0);
...
function FU($type,$args = array(),$is_full = false)
{
	...
	else
	{
		...
		switch($type)
		{
			...
			case 'book/cate':
			case 'book/shopping':
				$params = array('cate' => '','sid' => '','sort'=>'','tag' => '','page' => 0);
...
function tplFetch($file,&$args = array(), $tpl_dir = '',$cache_file = '')
{
	...
	if(!empty($cache_file) && !file_exists($cache_file) && diskfreespace(PUBLIC_ROOT.'./data/tpl/caches') > 1000000)
	{
		...
		if(isset($_FANWE['tpl_image_formats']))
			$dynamic_php .= "<?php\n".'$_FANWE[\'tpl_image_formats\']'." = ".var_export($_FANWE['tpl_image_formats'], true).";\n?>";
...
function express(&$content)
{
	...
	if(count($_FANWE['tpl_user_formats']) > 0)
	{
		...
		if(!empty($user_ids))
		{
			$res = FDB::query("SELECT uid,gid,user_name,avatar,reg_time,credits,is_daren,is_buyer,uc.* FROM ".FDB::table('user').' 
...
function setCache($key,$data,$expired_time = 0)
{
    ...
    $cache_data = "<?php\n".'$data[\''.$key."'] = ".var_export($cache_data, true).";\n\n?>";
...
function formatImage($img,$width=0,$height=0,$gen = 0,$is_full = 0)
{
	...
	if(strpos($img['src'],'./public/') === FALSE)
	{
		...
		{
			$img_url = FS("Image")->getServerUrl($img['src']);
			$args = array();
			$args['src'] = $img['src'];
			$args['width'] = $width;
			$args['height'] = $height;
			$args['gen'] = $gen;
			$args = authcode(serialize($args),'ENCODE',$authkey);
			$img_url .= 'getimg.php?args='.base64_encode($args);
		}
	}
	else
	{
		if(!file_exists(FANWE_ROOT.$img['src']))
			return '';
		...
		if(!file_exists(FANWE_ROOT.$img_url))
		{
			...
			$args['height'] = $height;
			$args['gen'] = $gen;
			$args = base64_encode(authcode(serialize($args),'ENCODE',$authkey));
			if($is_full)
				$img_url = $_FANWE['site_url']."getimg.php?args=".$args;
			else
				$img_url = $_FANWE['site_root']."getimg.php?args=".$args;
		}
		elseif($is_full)
...
function getUrlContent($url)
{
	...
	else
	{
		if(function_exists('curl_init'))
		{
			...
			//curl_setopt($ch, CURLOPT_REFERER,"http://www.mogujie.com/group");
